...
@inject('User')
@observer
export default class CarOut extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            modalImgUrl:null,
            queryType: [
                {key: "VehicleNO", val: "车辆号码"},
                {key: "VehicleSeries", val: "车辆品牌"},
                {key: "VehicleColor", val: "车身颜色"},
    ...
    doFetch = () => {
        ...
        CommonFetch.doFetch(RoutApi.getCarOutList, params, this.dealResponseData, this.refs.toast)
    }
    ...
    _renderItem = ({item, index}) => {
        return (
            <View style={[GlobalStyles.containerBg,styles.itemStyle,GlobalStyles.flexDirectRow, {alignItems:"center"}]}>
                <TouchableOpacity onPress={()=>this.showImageModal(item.ThroughSmartImgUrl)}>
                    <Image
                        style={{width: 146, height: 40, marginLeft: 20,resizeMode: 'contain'}}
                        resizeMode={'cover'}
                        source={item.ThroughSmartImgUrl?{uri: item.ThroughSmartImgUrl}:require("../../../assets/images/vehicle_blue.png")}
                    />
                </TouchableOpacity>
                <View>
                    <View style={[styles.itemRowStyle,{marginTop:15}]}>
                        <Text style={[GlobalStyles.font14Gray]}>车辆号码：</Text>
                        <Text style={[GlobalStyles.font14Gray]}>{item.VehicleNO}</Text>
                    </View>
                    <View style={styles.itemRowStyle}>
                        <Text style={[GlobalStyles.font14Gray]}>车身颜色：</Text>
                        <Text style={[GlobalStyles.font14Gray]}>{item.VehicleColor}</Text>
                    </View>
                    <View style={styles.itemRowStyle}>
                        <Text style={[GlobalStyles.font14Gray]}>车辆类型：</Text>
                        <Text style={[GlobalStyles.font14Gray]}>{item.VehicleType}</Text>
                    </View>
                    <View style={styles.itemRowStyle}>
                        <Text style={[GlobalStyles.font14Gray]}>车辆品牌：</Text>
                        <Text style={[GlobalStyles.font14Gray]}>{item.VehicleSeries}</Text>
                    </View>
                    <View style={styles.itemRowStyle}>
                        <Text style={[GlobalStyles.font14Gray]}>车辆系列：</Text>
                        <Text style={[GlobalStyles.font14Gray]}>{item.VehicleBrand}</Text>
                    </View>
                </View>
